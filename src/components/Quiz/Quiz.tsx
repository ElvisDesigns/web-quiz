...
export const TIME = 5; //laiks minutes
...
// eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const validateTest = (skipIncomplete: boolean = false) => {
        ...
        setMessage(`Apsveicam, tu ieguvi ${gottenPoints(calculatePoints())} no ${totalPoints()} punktiem`)
        window.scrollTo(0, 0);
    }
    ...
    return (
        <div>
            ...
            <button className={clsx('Quiz-Button')} onClick={onQuizButtonClick}>{ !submittedAnswers ? 'Nodot testu' : 'Sakt testu no jauna?'}</button>
        </div>
    )
}
